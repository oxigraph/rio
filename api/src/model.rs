...
/// assert_eq!(
///     "_:a1",
///     BlankNode::Named { id: "a1" }.to_string()
/// )
/// ```
#[derive(Eq, PartialEq, Debug, Clone, Copy, Hash)]
pub enum BlankNode<'a> {
    /// A "named" blank node, i.e. a blank node that has a [blank node identifier](https://www.w3.org/TR/rdf11-concepts/#dfn-blank-node-identifier) in the parsed content.
    Named {
        /// The [blank node identifier](https://www.w3.org/TR/rdf11-concepts/#dfn-blank-node-identifier).
        id: &'a str,
    },

    /// An "anonymous" blank node, i.e. a blank node without [blank node identifier](https://www.w3.org/TR/rdf11-concepts/#dfn-blank-node-identifier) in the parsed content.
    Anonymous {
        /// The [blank node identifier](https://www.w3.org/TR/rdf11-concepts/#dfn-blank-node-identifier) that have been automaticaly generated by the parser.
        id: u64,
    },
}
...
impl<'a> fmt::Display for BlankNode<'a> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            BlankNode::Named { id } => write!(f, "_:{}", id),
            BlankNode::Anonymous { id } => write!(f, "_:{}", id),
        }
    }
}
